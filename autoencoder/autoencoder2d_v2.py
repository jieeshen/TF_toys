...
def main():
    ...
    trainX_raw=np.asarray(df.drop(df.columns[[0]],axis=1))
    trainscaler=preprocessing.StandardScaler().fit(trainX_raw)
    trainX=trainscaler.transform(trainX_raw)
    print trainX_raw
    print trainX
...
#Build the encoder
    ...
    # 4th Layer
    W_fc4 = weight_variable([512, 128])
    b_fc4 = bias_variable([128])
    h_fc4 = tf.nn.sigmoid(tf.matmul(h_fc3_drop, W_fc4) + b_fc4)
    h_fc4_drop = tf.nn.dropout(h_fc4, keep_prob)

    #Code Layer
    W_fc5 = weight_variable([128, 2])
    b_fc5 = bias_variable([2])

    f_2 = tf.nn.sigmoid(tf.matmul(h_fc4_drop, W_fc5) + b_fc5)

# Reverse
    #decoder_Layer
    W_fc5_t=weight_variable([2, 128])
    b_fc5_t = bias_variable([128])

    h_fc5_t=tf.nn.sigmoid(tf.matmul(f_2, W_fc5_t) + b_fc5_t)
    h_fc5_t_drop = tf.nn.dropout(h_fc5_t, keep_prob)
    #4th layer
    W_fc4_t=weight_variable([128, 512])
    b_fc4_t = bias_variable([512])

    h_fc4_t=tf.nn.sigmoid(tf.matmul(h_fc5_t_drop, W_fc4_t) + b_fc4_t)
...
# regularizer
    regularizers=tf.nn.l2_loss(W_fc1)+tf.nn.l2_loss(W_fc2)+tf.nn.l2_loss(W_fc3)+tf.nn.l2_loss(W_fc4)+tf.nn.l2_loss(W_fc5)+\
                 tf.nn.l2_loss(W_fc1_t)+tf.nn.l2_loss(W_fc2_t)+tf.nn.l2_loss(W_fc3_t)+tf.nn.l2_loss(W_fc4_t)+tf.nn.l2_loss(W_fc5_t)
