...
#Build the encoder
    ...
    f_2 = tf.nn.sigmoid(tf.matmul(h_fc3_drop, W_fc4) + b_fc4)

# Reverse
    #decoder_Layer
    W_fc4_t=weight_variable([2, 512])
    b_fc4_t = bias_variable([512])

    h_fc4_t=tf.nn.sigmoid(tf.matmul(f_2, W_fc4_t) + b_fc4_t)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(nepoch+1):
            ...
            if (epoch) % 10 ==0:
                ...
                ft_df.to_csv("out_"+str(epoch)+".csv", sep=" ",index=True)
